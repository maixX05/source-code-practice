package com.msr.better.redis.bloomflter;
...
public class RedisBloomFilter {
    ...
    private static RedisScript<Boolean> bfreserveScript = new DefaultRedisScript<>(
            "return redis.call('bf.reserve', KEYS[1], ARGV[1], ARGV[2])", Boolean.class);

    private static RedisScript<Boolean> bfaddScript = new DefaultRedisScript<>(
            "return redis.call('bf.add', KEYS[1], ARGV[1])", Boolean.class);

    private static RedisScript<Boolean> bfexistsScript = new DefaultRedisScript<>(
            "return redis.call('bf.exists', KEYS[1], ARGV[1])", Boolean.class);
    ...
    public void bloomTest(){
        // 配置错误率和存储空间
        // System.err.println(bloom.bfReserve("b3", 0.0001, 5000));
        // 添加元素
        // System.err.println(bloom.bfAdd("b3", "7"));
        // 判断元素是否存在
        // System.err.println(bloom.bfExists("b3", "8"));
        // 批量添加元素
        // System.err.println(bloom.bfmadd("b3", "8", "9" , "12", "13"));
        // 批量判断元素是否存在
        // System.err.println(bloom.bfmexists("b3", "8", "9", "13", "14"));
    }
}
